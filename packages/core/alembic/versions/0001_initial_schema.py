...
def upgrade() -> None:
    ...
    chunking_strategy.create(op.get_bind(), checkfirst=True)
    answer_status.create(op.get_bind(), checkfirst=True)
    golden_status.create(op.get_bind(), checkfirst=True)
    eval_verdict.create(op.get_bind(), checkfirst=True)
    ...
    op.create_index("ix_documents_fingerprint", "documents", ["fingerprint"], unique=True)
    ...
    # pgvector column + HNSW index (1536 dims = text-embedding-3-small)
    ...
    op.create_table(
        ...
        sa.Column(
            "verdict",
            sa.Enum("pass", "regression", name="evalverdict", create_type=False),
            nullable=True,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "scope",
            sa.Enum("query", "day", name="budgetscope", create_type=False),
            nullable=False,
        ),
        sa.Column("cap_usd", sa.Numeric(10, 4), nullable=False),
        sa.Column("consumed_usd", sa.Numeric(10, 4), nullable=False, server_default="0"),
